...
def DrawIris(image, lefteye, righteye):
    cv2.circle(image,(lefteye[0],lefteye[1]),lefteye[2],(0,0,255),3)
    cv2.circle(image,(righteye[0],righteye[1]),righteye[2],(0,0,255),3)

def get_iris(image, shape):
        ...
        righteye = selected_circle_right
...
if __name__ == "__main__":
    ...
    out = glob.glob( scriptDir + os.path.sep + folder_name + os.path.sep + "*.jpg" ) 
    print(out)
    print(scriptDir)
    n = 0
    for f in out:
        ...
        np.savetxt(folder_name+os.path.sep+filename+".csv", shape, delimiter=',', fmt='%d')
        cv2.imwrite(folder_name+os.path.sep+filename+"_out.jpg",image)        
        n = n +1
